...
import os, sys, time, json, zipfile, logging, threading, asyncio
import shutil, random, string, urllib.parse
...
try:
    ...
    print("Error: checker.py not found.")
    sys.exit(1)
...
# ──────────────────────────────────────────────────────────────────────────
#  BOT STATE, SESSION, LIVE STATS (same as before – compact version)
# ──────────────────────────────────────────────────────────────────────────
class BotState:
    def __init__(self, user_id: int, chat_id: int, threads: int, stop_event: threading.Event):
        self.user_id = user_id; self.chat_id = chat_id; self.threads = threads
        ...
        self.is_running = True; self.finished = False
    def should_stop(self): return self.stop_event.is_set()
    def finish(self): self.is_running = False; self.finished = True

class CheckerSession:
    def __init__(self, user_id, chat_id, combo_file, threads):
        self.user_id = user_id; self.chat_id = chat_id; self.combo_file = combo_file
        self.threads = threads; self.stop_event = threading.Event()
        self.state = BotState(user_id, chat_id, threads, self.stop_event)
        self.thread = None; self.live_message_id = None
        self.finished = False; self.is_running = True
        self.combo_stem = Path(combo_file).stem
    def stop(self):
        self.stop_event.set()
        if self.thread and self.thread.is_alive(): self.thread.join(timeout=5)
        self.is_running = False
    def mark_finished(self): self.is_running = False; self.finished = True
    def get_stats(self): return self.state.live_stats.snap()
...
def send_results_background(session: CheckerSession):
    try:
        ...
        if not rm or not rm.base.exists(): return
...
def safe_send_message(chat_id, text, parse_mode=ParseMode.HTML, **kwargs):
    ...
    if not app_instance or not event_loop: return None
    ...
    try: return fut.result(timeout=25)
    except Exception as e: LOG.error(f"send failed: {e}"); return None

def safe_edit_message(chat_id, message_id, text):
    global app_instance, event_loop
    if not app_instance or not event_loop: return False
    coro = app_instance.bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text, parse_mode=ParseMode.HTML)
    fut = asyncio.run_coroutine_threadsafe(coro, event_loop)
    try: fut.result(timeout=15); return True
    except Exception as e: LOG.debug(f"edit failed: {e}"); return False

def safe_send_document(chat_id, document, filename, caption=""):
    global app_instance, event_loop
    if not app_instance or not event_loop: return
...
def send_hit_to_user(chat_id, result):
    if not result.get('has_codm'): return
    acc = result.get('account',''); pwd=result.get('password','')
    clean=result.get('is_clean',False); lvl=result.get('codm_level','N/A')
    region=result.get('codm_region','N/A'); nick=result.get('codm_nickname','N/A')
    shell=result.get('shell_balance',0); country=result.get('country','N/A')
...
def send_final_summary(chat_id, ls: LiveStats, stopped: bool):
    ...
    lines = ["🏁 <b>Checking Complete</b>" if not stopped else "🛑 <b>Checking Stopped</b>", "",
...
def send_result_files(chat_id, rm: ResultsManager):
    base = rm.base
    if not base.exists(): safe_send_message(chat_id, "ℹ️ No result files to send."); return
    zip_parts = zip_results_folder(base, base / "results.zip")
    if not zip_parts: safe_send_message(chat_id, "ℹ️ No result files to send."); return
    total = len(zip_parts)
    for idx, zp in enumerate(zip_parts, 1):
        caption = f"📦 Results part {idx}/{total}" if total > 1 else "📦 Your results"
        with open(zp, "rb") as f:
...
def log_terminal(result): pass
def process_next(): pass
...
def run_checker_thread(session: CheckerSession):
    try:
        ...
        try:
            with open(session.combo_file, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    ...
                    if acc and pwd: accounts.append((acc, pwd))
        except Exception as e:
            safe_send_message(session.chat_id, f"❌ Failed to read file: {e}")
            session.mark_finished(); return
        if not accounts:
        ...
        with session_lock:
            if session.user_id in active_sessions: del active_sessions[session.user_id]
...
def live_stats_updater(session: CheckerSession):
    ...
    try:
        ...
        if msg_id: session.live_message_id = msg_id
    except Exception as e:
        LOG.error(f"initial live failed: {e}"); return
    fail_count = 0; last_proc = -1
    while session.is_running and not session.finished:
        time.sleep(LIVE_INTERVAL)
        if session.finished or not session.is_running: break
        try:
            text = build_live_text(session)
            ...
            if session.live_message_id:
                ...
                if not ok:
                    ...
                    if fail_count >= 3:
                        ...
                        if new_id: session.live_message_id = new_id; fail_count = 0
                else: fail_count = 0
        except Exception as e: LOG.debug(f"live loop: {e}")
...
def build_live_text(session: CheckerSession) -> str:
    ...
    bar_len = 20; filled = int(pct / 100 * bar_len)
    bar = "█" * filled + "░" * (bar_len - filled)
    lines = [f"⚡ <b>CODM Checker</b>  <code>#{session.user_id}</code>",
             f"`[{bar}] {pct:.1f}%`",
...
async def cmd_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if activate_key(user_id, formatted):
        expiry = get_user_expiry(user_id)
        await update.message.reply_text(f"✅ <b>Key activated!</b>\n📅 Valid until: <code>{format_ph_time(expiry)}</code>", parse_mode=ParseMode.HTML)
    else:
...
async def cmd_mykey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    minutes = rem // 60
    await update.message.reply_text(f"🔑 <b>Your license</b>\n🕒 Valid until: <code>{format_ph_time(expiry)}</code>\n⏳ Remaining: <b>{int(hours)}h {int(minutes)}m</b>", parse_mode=ParseMode.HTML)
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Check started with {DEFAULT_THREADS} threads.\nFile: <code>{doc.file_name}</code>", parse_mode=ParseMode.HTML)
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        ...
        return
    keyboard = [[InlineKeyboardButton("🔑 Generate Key", callback_data="admin_genkey")],
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_genkey":
        await query.edit_message_text("⏳ Use /genkey &lt;duration&gt;  (e.g. 1h, 7d, 3600)", parse_mode=ParseMode.HTML)
...
async def cmd_genkey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Key generated:\n<code>{key}</code>\n⏳ {seconds//3600}h {(seconds%3600)//60}m", parse_mode=ParseMode.HTML)
...
async def handle_broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get('broadcast_mode'): return
    user_id = update.effective_user.id
