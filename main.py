#!/usr/bin/env python3
# =============================================================================
#  main.py  —  CODM Telegram Checker Bot  v5.3
...
#
#  NEW in v5.3:
#    - Auto-harvest fresh_cookie.txt on session start if missing
#      (DataDome / Garena harvester, adapted from Solar Tools cookie.py)
#    - Refresh every session (configurable via bot_config.json)
#    - Admin command /refreshcookies + admin panel button
#
#  Admin ID : 8621676055
...
# Cookie harvester defaults
COOKIE_HARVEST_COUNT   = 15      # target cookies to collect
COOKIE_HARVEST_WORKERS = 20      # parallel workers
COOKIE_HARVEST_TIMEOUT = 60      # hard ceiling in seconds
COOKIE_HARVEST_RETRIES = 2       # attempts per cookie
...
# =============================================================================
#  Behavior:
#    - On every bot startup, ensure_fresh_cookies() is called.
#    - If fresh_cookie.txt is MISSING  → harvest cookies and write the file.
#    - If fresh_cookie.txt EXISTS      → refresh it (harvest again).
#    - Harvesting is done in parallel; results are de-duplicated.
#    - Never crashes the bot on network/parse errors — logs and continues.
...
def _fetch_one_datadome(session: "requests.Session | None" = None) -> str | None:
    ...
    Adapted from cookie.py::get_datadome().
    """
...
def _atomic_write_lines(path: str, lines: list[str]) -> bool:
    """Write lines atomically (tmp + os.replace)."""
    try:
        tmp = path + ".tmp"
...
def harvest_cookies(
    target_count: int = COOKIE_HARVEST_COUNT,
    ...
    # We allow more attempts than target to compensate for failures.
    max_attempts = max(target_count * 3, target_count + 10)

    def _task(_i: int) -> str | None:
        for _attempt in range(COOKIE_HARVEST_RETRIES):
    ...
    logger.info(
        f"[COOKIES] Harvesting up to {target_count} cookies "
        f"({max_workers} workers, {timeout}s ceiling)..."
    )
    ...
    logger.info(
        f"[COOKIES] Harvest complete: {len(collected)} cookie(s) "
        f"in {time.time() - start_ts:.1f}s."
    )
    return collected

def fetch_fresh_cookies() -> bool:
    """
    Harvest fresh cookies and write fresh_cookie.txt atomically.
    Returns True on success (file written with >=1 cookie), else False.
    """
    # Configurable overrides via bot_config.json
    target  = int(cfg.get("cookie_harvest_count",   COOKIE_HARVEST_COUNT))
    ...
    if not cookies:
        logger.error("[COOKIES] Harvest returned 0 cookies.")
        return False

    if not _atomic_write_lines(FRESH_COOKIE_FILE, cookies):
        return False

    logger.info(f"[COOKIES] fresh_cookie.txt written ({len(cookies)} cookies).")
    return True

def ensure_fresh_cookies() -> bool:
    """
    Called once per session (bot startup).
    - If fresh_cookie.txt is missing → harvest.
    - If it exists → refresh it anyway (every session).
    - Never raises.
    """
    exists = os.path.exists(FRESH_COOKIE_FILE)
    if not exists:
        logger.info("[COOKIES] fresh_cookie.txt not found — harvesting...")
    else:
        logger.info("[COOKIES] Refreshing fresh_cookie.txt for this session...")

    ok = fetch_fresh_cookies()

    if not ok and not exists:
        logger.warning(
            "[COOKIES] No fresh cookies available. Bot will continue "
            "without them; CookieManager may use cached/empty set."
        )
    elif not ok and exists:
        logger.warning(
            "[COOKIES] Refresh failed — keeping previous fresh_cookie.txt."
        )
    return ok
...
def cmd_help(message):
    ...
    if is_admin(message.from_user.id):
        lines += [
            ...
            "<b>/refreshcookies</b>                          — Re-harvest fresh_cookie.txt",
        ]
    ...
    if fresh_exists:
        try:
            ...
            with open(FRESH_COOKIE_FILE, "r", encoding="utf-8", errors="ignore") as f:
                fresh_lines = sum(1 for _ in f if _.strip())
        except Exception:
            pass
...
def cmd_admin(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🍪 Refresh Cookies", callback_data="adm:refreshcookies"),
    ...
    if fresh_exists:
        try:
            ...
            pass
    fresh_line = f"\n<b>fresh_cookie:</b> {'✅' if fresh_exists else '❌'} ({fresh_lines})"
...
def cb_admin(call):
    ...
    if action == "stats":
        ...
        bot.send_message(
            call.message.chat.id,
            "🍪 <b>Harvesting fresh cookies…</b>\nThis may take a few seconds.",
            parse_mode="HTML",
        )
        ok = fetch_fresh_cookies()
        if ok:
            size = os.path.getsize(FRESH_COOKIE_FILE) if os.path.exists(FRESH_COOKIE_FILE) else 0
            try:
                with open(FRESH_COOKIE_FILE, "r", encoding="utf-8", errors="ignore") as f:
                    cnt = sum(1 for _ in f if _.strip())
            except Exception:
                cnt = 0
            bot.send_message(
                call.message.chat.id,
                f"✅ <b>fresh_cookie.txt refreshed.</b>\n"
                f"<b>Cookies:</b> {cnt}\n<b>Size:</b> {size} bytes",
                parse_mode="HTML",
            )
        else:
            bot.send_message(
                call.message.chat.id,
                "❌ <b>Failed to harvest fresh_cookie.txt.</b>\nSee server logs for details.",
                parse_mode="HTML",
            )
...
def main():
    ...
    console.print()
    console.print(Panel(
        Text("CODM Account Checker Bot  v5.3", style="bold bright_magenta") +
    ...
    # ── FRESH COOKIES: harvest if missing + refresh every session ──
    console.print("  [bright_cyan]⬡[/bright_cyan]  [dim]Ensuring fresh_cookie.txt...[/dim]")
    try:
        cookies_ok = ensure_fresh_cookies()
        if cookies_ok:
            size = os.path.getsize(FRESH_COOKIE_FILE) if os.path.exists(FRESH_COOKIE_FILE) else 0
            try:
                with open(FRESH_COOKIE_FILE, "r", encoding="utf-8", errors="ignore") as f:
                ...
                cnt = 0
            console.print(
                f"  [bright_green]✔[/bright_green]  [dim]fresh_cookie.txt ready "
                f"({cnt} cookies, {size} bytes)[/dim]"
            )
        else:
            console.print(
                "  [yellow]⚠[/yellow]  [dim]fresh_cookie.txt not refreshed — "
                "harvester may have failed[/dim]"
            )
    except Exception as e:
        console.print(f"  [yellow]⚠[/yellow]  [dim]Cookie init error: {e}[/dim]")
