#!/usr/bin/env python3
# =============================================================================
#  main.py  —  CODM Telegram Checker Bot  v5.0
#  Clean version: time‑based keys, no feedback system
...
BOT_TOKEN = "8597114754:AAH3nvgyXWg1KpQq1_Qn2Lva2J0yozUJxGc"   # <-- YOUR TOKEN HERE
...
# =============================================================================
#   KEYS & USERS DB  (time‑based keys only)
# =============================================================================
_keys_lock  = Lock()
_users_lock = Lock()
...
def gen_key(days: int = 0, created_by: int = 0) -> str:
    """Generate a time‑based key with a validity in days."""
    chars = string.ascii_uppercase + string.digits
    key   = "CODM-" + "-".join("".join(random.choices(chars, k=5)) for _ in range(4))
    keys  = load_keys()
    now   = datetime.now()
    expiry = (now + timedelta(days=days)).isoformat() if days > 0 else None
    keys[key] = {
        "days":         days,
    ...
    if uid in ADMIN_IDS:
        ...
        try:
            ...
            if exp_dt > datetime.now():
                key_status = f"🔑 <b>Active key</b> — expires {exp_dt.strftime('%Y-%m-%d %H:%M')}"
            else:
...
def cmd_help(message):
    ...
    if is_admin(message.from_user.id):
        lines += [
            ...
            "<b>/genkey &lt;days&gt; [count]</b>                   — Generate time‑based key(s)",
...
def cmd_mykey(message):
    ...
    try:
        ...
        if exp_dt < datetime.now():
            ...
            status = f"✅ <b>Active</b> — {days_left}d remaining"
    except Exception:
    ...
    if uid in ADMIN_IDS:
        ...
        if expiry:
            try:
                ...
                key_status = f"🔑 Active — {days_left}d remaining"
            except:
...
def _send_listkeys(chat_id):
    ...
    for k, v in list(keys.items()):
        ...
        expiry_s = f"{days_v}d" if days_v > 0 else "Lifetime"
...
def _send_listusers(chat_id):
    ...
    for uid, u in list(users.items())[:20]:
        ...
        lines.append(f"{banned}{has_key} <code>{uid}</code> @{name}")
    if len(users) > 20:
...
def cmd_genkey(message):
    ...
    if len(parts) < 2:
        bot.send_message(
            message.chat.id,
            "Usage: <code>/genkey &lt;days&gt; [count]</code>\n"
            "Example: <code>/genkey 30</code>  or  <code>/genkey 7 3</code>",
            parse_mode="HTML",
        )
        return
    try:
        ...
        return
    out = [f"<b>🔑 Generated {count} key(s) — {days} days each</b>", "━" * 30]
    for _ in range(count):
        out.append(f"<code>{gen_key(days, created_by=message.from_user.id)}</code>")
...
def main():
    ...
    console.print()
    console.print(Panel(
        Text("CODM Account Checker Bot  v5.0", style="bold bright_magenta") +
