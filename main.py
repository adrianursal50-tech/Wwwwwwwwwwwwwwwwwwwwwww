#!/usr/bin/env python3
# =============================================================================
#  main.py  —  CODM Telegram Checker Bot  v5.4
...
#
#  NEW in v5.4:
#    - Background cookie harvester that runs CONTINUOUSLY
#    - Pool is auto-topped-up so EVERY account check gets a fresh cookie
#    - No manual refresh needed — it self-replenishes
#    - Live CookieManager reload after each harvest batch
#
#  Admin ID : 8621676055
...
# ── Cookie pool defaults (all overridable via bot_config.json) ─────────────
COOKIE_POOL_TARGET     = 500    # try to keep this many cookies in fresh_cookie.txt
COOKIE_POOL_MIN        = 100    # if pool drops below this → urgent refill
COOKIE_POOL_BATCH      = 50     # how many to harvest per refill cycle
COOKIE_HARVEST_WORKERS = 20     # parallel workers per harvest
COOKIE_HARVEST_TIMEOUT = 60     # seconds ceiling per harvest cycle
COOKIE_POOL_IDLE_WAIT  = 5      # seconds between pool checks when full
COOKIE_POOL_LOW_WAIT   = 2      # seconds between pool checks when low
...
# =============================================================================
#   FRESH COOKIE HARVESTER  (DataDome / Garena)
#   Adapted from Solar Tools cookie.py (@Maisanyvokei · @solargng)
# =============================================================================
#  Runtime model (v5.4):
#    - A BACKGROUND daemon thread runs for the entire bot lifetime.
#    - It checks the size of fresh_cookie.txt every few seconds.
#    - If the pool is below COOKIE_POOL_TARGET, it harvests a batch and
#      appends (atomically) to fresh_cookie.txt — DEDUPLICATED.
#    - After every batch, the live CookieManager is reloaded, so the next
#      account that runs in run_check() sees the fresh pool.
#    - Result: for ALL practical purposes, EVERY account check pulls a
#      fresh cookie without any manual action from the user or admin.
#
#  Manual triggers still exist (admin /refreshcookies + panel button) but
#  the bot never *needs* them — it self-replenishes.
...
def _fetch_one_datadome(session: "requests.Session | None" = None) -> str | None:
    """
    Fetch a single DataDome cookie from Garena.
    Returns the raw cookie value (e.g. 'datadome=...') or None on failure.
    """
...
def _count_fresh_cookies() -> int:
    """Count non-empty lines in fresh_cookie.txt (thread-safe read)."""
    try:
...
def _append_cookies_atomic(new_lines: list[str]) -> int:
    """
    Append new cookie lines to fresh_cookie.txt atomically (tmp + os.replace),
    de-duplicating against existing content. Returns count of lines actually added.
    """
    if not new_lines:
        return 0

    with _cookie_pool_lock:
        # Load existing (ordered, deduped)
        existing_ordered: dict = {}
...
def harvest_cookies(
    target_count: int = COOKIE_POOL_BATCH,
    max_workers: int = COOKIE_HARVEST_WORKERS,
    timeout: int = COOKIE_HARVEST_TIMEOUT,
) -> list[str]:
    """
    Harvest cookies in parallel until target_count is reached or timeout hits.
    Returns a de-duplicated list of cookie lines (e.g. 'datadome=...').
    """
    collected: list[str] = []
    seen: set[str] = set()
    collected_lock = Lock()
...
def fetch_fresh_cookies(target_override: int | None = None) -> int:
    """
    Harvest one batch and append to fresh_cookie.txt.
    Returns number of NEW cookies actually added (0 on failure).
    """
    ...
    added = _append_cookies_atomic(cookies)
    return added

def _reload_live_cookie_manager() -> None:
    """
    After a harvest, try to reload the live CookieManager so newly added
    cookies take effect immediately for accounts already being processed.
    Tries common reload method names first; falls back to re-instantiating.
    """
    try:
        cm = STATE.cookie_mgr
        if cm is None:
...
def _cookie_harvester_loop() -> None:
    """
    Background daemon: keeps fresh_cookie.txt topped up FOREVER.
    This is what makes 'fetch a cookie for every account' work automatically.
    """
        ...
        try:
            ...
            if current < target:
                ...
                if fetch_n > 0:
                    ...
                    if added > 0:
                        logger.info(
                            f"[COOKIES] +{added} new cookies → pool now {total}."
                        )
                        _reload_live_cookie_manager()
                    else:
                        logger.debug("[COOKIES] Harvest produced 0 new cookies.")
...
def ensure_fresh_cookies() -> bool:
    """
    Synchronous seed at startup: if pool is empty, do ONE blocking harvest
    so the first check doesn't wait. After that the background thread
    keeps it topped up automatically.
    """
    current = _count_fresh_cookies()
    if current > 0:
...
def _ensure_pool_for(account_count: int) -> None:
    """
    Best-effort: if the pool is smaller than the number of accounts about to
    be checked, kick an on-demand harvest in a background thread so the pool
    can catch up. Never blocks the check from starting.
    """
    current = _count_fresh_cookies()
...
def _process_next_queue():
    ...
    try:
        ...
        # Ensure pool is at least as large as the check (best-effort, non-blocking)
...
def main():
    ...
    console.print()
    console.print(Panel(
        Text("CODM Account Checker Bot  v5.4", style="bold bright_magenta") +
    ...
    # ── Seed cookie pool if empty (blocking, one-time) ──
    console.print("  [bright_cyan]⬡[/bright_cyan]  [dim]Seeding fresh_cookie.txt...[/dim]")
    try:
        seeded = ensure_fresh_cookies()
        current = _count_fresh_cookies()
        if current > 0:
            console.print(
    ...
    # ── Start background auto-harvester (the "per-account" magic) ──
    try:
        harvester = Thread(
