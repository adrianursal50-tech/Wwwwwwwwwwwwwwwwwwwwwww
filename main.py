#!/usr/bin/env python3
# =============================================================================
#  main.py  —  CODM Telegram Checker Bot  v5.5
#  Engine: TOSHIPRIVATE.py (imported inline — no checker.py shim)
# =============================================================================
...
class ProxyManager(_engine.ProxyManager):
    def loaded(self) -> bool: return self.is_loaded()
    def count(self)  -> int:  return self.get_count()
...
class ResultsManager(_engine.ResultsManager):
    def __init__(self, combo_file_path, user_id=None, create_dirs=True):
        ...
        if user_id is not None:
            super().__init__(stem, create_dirs=False)
            ...
            super().__init__(stem, create_dirs=create_dirs)
    ...
    def get_result_files(self) -> dict:
        ...
        for fp in sorted(self.base_dir.glob("*.txt")):
            out[fp.stem.replace(f"_{self.timestamp}", "")] = fp
        for sub in ("Country", "Level", "Garena Shells"):
            sd = self.base_dir / sub
            if sd.exists():
                for fp in sorted(sd.glob("*.txt")):
                    out[f"{sub}/{fp.stem.replace(f'_{self.timestamp}', '')}"] = fp
        return out
        ...
        if not hasattr(_tls, "sess"):
            ...
            if cks:
                ...
                try:
                    ...
                    if dd:
                        _tls.dd.set_datadome(dd)
                        _tls.sess.cookies.set("datadome", dd, domain=".garena.com")
                except Exception:
                    pass
...
def activate_key(user_id: int, key_val: str) -> bool:
    with _keys_lock:
        ...
        users[uid]["is_starter"]   = users[uid].get("is_starter", False)
        save_users(users)
        return True

def grant_free_hour(user_id: int) -> bool:
    key = gen_key(duration_seconds=3600, created_by=0)
...
def main():
    ...
    console.print()
    console.print(Panel(
        Text("CODM Account Checker Bot  v5.5", style="bold bright_magenta") +
    ...
    console.print(Panel(
        ...
        f"[bright_cyan]⬡[/bright_cyan]  Engine:   [dim]TOSHIPRIVATE.py[/dim]",
